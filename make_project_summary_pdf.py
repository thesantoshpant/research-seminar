...
# ---- Image paths ---------------------------------------------------------
UNET_CM     = ROOT / "archive" / "runs" / "unet_imgonly_v1" / "confmat.png"
...
# ---- Helpers -------------------------------------------------------------
def img(path: Path, width_in: float = 5.0):
    """Return a Flowable image if path exists, else a note."""
    if not path.exists():
        return Paragraph(f"<i>(image missing: {path.name})</i>", CAP)
    return Image(str(path), width=width_in * inch, height=width_in * inch * 0.75,
                 kind="proportional")


def metrics_table(rows):
    """rows: list of (label, value) tuples. Returns a Table flowable."""
    tbl = Table(rows, colWidths=[2.2 * inch, 3.0 * inch])
...
# Title page
story.append(Paragraph("Sea Ice Classification &mdash; Project Summary", H1))
story.append(Paragraph(
    ...
    "<b>water</b>. We trained on two Antarctic tiles "
    "(T02CNA, T02CNC) and tested on a third never-seen tile (T03CWT). The "
    "scores below are all measured on that held-out test tile.",
    BODY))
...
# Headline table
story.append(Paragraph("Headline results (test tile T03CWT)", H2))
...
# ----- 1. U-Net -----
story.append(Paragraph("1. U-Net &mdash; image only", H1))
story.append(Paragraph(
    ...
    "gives the model a head-start: it already knows how to recognize "
    "edges, textures, and shapes before it even sees sea ice.",
    BODY))
story.append(Paragraph(
...
story.append(Paragraph("Benchmarks", H2))
story.append(metrics_table([
    ["Metric", "Value"],
...
story.append(Spacer(1, 0.15 * inch))
story.append(Paragraph("Confusion matrix &mdash; U-Net", H2))
story.append(img(UNET_CM, width_in=4.5))
story.append(Paragraph(
...
story.append(Paragraph("Sample predictions &mdash; U-Net", H2))
story.append(img(UNET_PREDS, width_in=5.5))
story.append(Paragraph(
    "Left column: the satellite image the model saw. Middle: the "
    "human-drawn ground truth. Right: what U-Net predicted. "
    "Red = thick ice, blue = thin ice, green = water.", CAP))

story.append(PageBreak())

# ----- 2. LSTM -----
story.append(Paragraph("2. LSTM &mdash; photon CSV only", H1))
story.append(Paragraph(
    ...
    "and remembers context as it goes. Perfect for time-series or, in "
    "our case, a series of 10-meter segments along a ICESat-2 ground "
    "track.",
    BODY))
story.append(Paragraph(
    "We followed the professor&rsquo;s own LSTM recipe (her Keras "
    "notebook for the same problem):",
    BODY))
story.append(metrics_table([
    ["Setting",                "Value"],
...
story.append(Paragraph(
    "We didn&rsquo;t guess these numbers &mdash; we ran a 21-config "
    "sweep that varied the alpha vector, gamma, hidden size, learning "
    "rate, dropout, window length and random seed one axis at a time. "
    "The winner was <b>hidden = 96</b> &mdash; everything else stayed "
    "close to the professor&rsquo;s defaults. The 8 features are "
    "physically meaningful: mean / median / standard-deviation of "
    "corrected photon heights, photon counts, background counts, "
    "background rate, and two derived height-asymmetry features.",
    BODY))
...
story.append(Spacer(1, 0.15 * inch))
story.append(Paragraph("Confusion matrix &mdash; LSTM", H2))
story.append(img(LSTM_CM, width_in=4.5))
story.append(Paragraph(
    ...
    "no idea what the surrounding 128&times;128 image actually looks "
    "like.", CAP))

story.append(PageBreak())

# ----- 3. Deep Fusion (winner) -----
story.append(Paragraph("3. Deep Fusion &mdash; image + LSTM &mdash; the winner", H1))
story.append(Paragraph(
    "This is the model that won. It uses both data sources, and it lets "
    "the two networks compare notes inside the architecture (not just "
    "by averaging their predictions at the end).",
    BODY))
story.append(Paragraph(
    "<b>How it works:</b>",
    BODY))
story.append(Paragraph(
    "&bull; The image branch is the same U-Net from Model 1, which "
    "turns the satellite picture into a feature map. <br/>"
    "&bull; The CSV branch is the <b>LSTM winner from Model 2</b>, "
    "loaded with the weights it learned during the sweep. <br/>"
    ...
    "model can up- or down-weight either branch depending on what it "
    "sees. <br/>"
    "&bull; A final 3&times;3 convolution outputs the per-pixel "
    "class prediction.",
    BODY))
story.append(Paragraph(
    ...
    "and let them adapt slowly during fusion training (10x slower than "
    "the rest of the network). This &ldquo;two pretrained experts get "
    "glued together&rdquo; recipe gave us +0.01 mIoU over starting from "
    "scratch.",
    BODY))
...
story.append(Spacer(1, 0.15 * inch))
story.append(Paragraph("Confusion matrix &mdash; Deep Fusion (winner)", H2))
story.append(img(FUSION_CM, width_in=4.5))
story.append(Paragraph(
    "The diagonals are 96 / 92 / 96 &mdash; none below 92%. "
    "Compared to the LSTM alone (97 / 76 / 85), fusion lifted thin "
    "ice by +16 percentage points and water by +11 pp without "
    "losing thick ice.",
    CAP))

story.append(Paragraph("Training curves", H2))
story.append(img(FUSION_LOSS, width_in=6.2))
story.append(Paragraph(
...
story.append(Paragraph("Side-by-side comparison", H1))
story.append(Paragraph(
    "All three models scored on the same held-out test tile (T03CWT):",
    BODY))

# Bigger comparison table
cmp_rows = [
...
story.append(Paragraph(
    ...
    "(0.87 mIoU). It handles thick ice and water well but struggles "
    "with thin ice because thin ice looks visually similar to thick "
    "ice in the satellite RGB.",
    BODY))
story.append(Paragraph(
    "<b>2.</b> The photon-only LSTM is weak on its own (0.70 mIoU), "
    "but the photons carry real signal about ice <i>thickness</i> "
    "that the image can&rsquo;t see. The LSTM learns to use that.",
    BODY))
story.append(Paragraph(
    "<b>3.</b> When you fuse them deeply (not just averaging "
    "predictions, but combining the internal feature maps with "
    "attention), the two modalities cover each other&rsquo;s blind "
    "spots. Result: <b>0.90 mIoU</b>, with the biggest gains on the "
    "two minority classes (thin ice +16 pp, water +11 pp vs LSTM "
    "alone; thin ice +5 pp vs U-Net alone).",
    BODY))
story.append(Paragraph(
...
story.append(Paragraph(
    ...
    "T03CWT, which the model never saw during training. The training "
    "set was tiles T02CNA and T02CNC. Code, notebooks, and per-run "
    "metrics live under runs/ (winners) and archive/ (everything else "
    "we tried).</i>",
    SMALL))
