"""
Generate fusion_v3.ipynb -- v2's prof-style *data* pipeline (5-seg
window, 8 engineered features, focal loss) but with v1's LSTM *capacity*
(Bi-LSTM(128) x 2 layers, bidirectional) and v1's stable LR (1e-4).

v2 lost to v1 by 0.09 mIoU. Two likely culprits:
  (a) v2's LSTM was ~10x smaller than v1's
  (b) v2's LR was ~9x higher -> destabilized ResNet-18 fine-tuning
       (val loss spiked to 5x floor at epoch 15)

v3 holds capacity AND LR equal to v1 and only changes the *data feed*
and the *loss function*. Any remaining gap to v1 is then attributable
to "5-seg / 8 engineered features + focal loss" vs "32-seg / ~25 raw
features + weighted CE".

    Bi-LSTM(hidden=128), 2 layers, dropout=0.4, bidirectional
    Input: 5-segment along-track window over 8 engineered features
    Head:  Dense(256->16, ELU) + Dropout(0.4) + Dense(16->16, ELU)
           -> tile to (16, 128, 128)
    Loss:  CategoricalFocalLoss(alpha=[0.05,0.45,0.60], gamma=2.0)
    LR:    1e-4 (matches v1 -- stable for ResNet-18 fine-tuning)

Outputs (in runs/fusion_v3/):
  - best.pt, final.pt, metrics.csv
  - test_metrics.json
  - confmat.png  (prof-style percentages)
  - loss_curve.png
  - summary_vs_v1.csv  -- v1 vs v2 vs v3 vs LSTM-only winner
"""

from pathlib import Path
import nbformat as nbf

OUT = Path(__file__).parent / "fusion_v3.ipynb"
...
# =========================================================================
md(r"""# Deep Fusion v3 -- prof-style data + focal loss + v1 capacity

v2 (uni-LSTM(96), LR 8.886e-4) lost to v1 by 0.09 mIoU and trained
unstably. v3 keeps v2's prof-style data feed (5-segment window over 8
engineered features) and focal loss, but uses v1's LSTM capacity
(Bi-LSTM(128) x 2 layers) and v1's stable LR (1e-4). This isolates
the *data and loss* changes from the *capacity / LR* changes.
...
| component | v1 (`fusion_deep`) | v2 (lost) | v3 (this notebook) |
|---|---|---|---|
| CSV window | 32 segments | **5 segments** | **5 segments** |
| CSV features | ~25 raw cols | **8 engineered** | **8 engineered** |
| LSTM | Bi-LSTM(128) x 2 | uni-LSTM(96) x 1 | **Bi-LSTM(128) x 2 (= v1)** |
| Head | Linear(256->16) | Dense(96->16,elu)x2 | **Dense(256->16,elu)x2** |
| Loss | weighted CE | Focal alpha,gamma | **Focal alpha=[.05,.45,.60] gamma=2.0** |
| Optimizer LR | **1e-4** | 8.886e-4 (unstable) | **1e-4 (= v1)** |
| Image branch | U-Net ResNet-18 | unchanged | unchanged |
| Fusion | SE + 3x3 head | unchanged | unchanged |""")
...
LSTM_HIDDEN  = 128          # matches v1
LSTM_LAYERS  = 2            # matches v1
LSTM_BIDIR   = True         # matches v1
LSTM_DROPOUT = 0.4          # prof's value (v1 had 0.2)
HEAD_HIDDEN  = 16
HEAD_DROPOUT = 0.4
...
class FusionV3Model(nn.Module):
    def __init__(self, n_features=N_FEATS, num_classes=NUM_CLASSES, patch=PATCH,
        ...
        # CSV branch -- v1-capacity Bi-LSTM(128) x 2 layers
        self.lstm = nn.LSTM(input_size=n_features, hidden_size=lstm_hidden,
                            num_layers=lstm_layers, batch_first=True,
                            bidirectional=lstm_bidir,
                            dropout=lstm_dropout if lstm_layers > 1 else 0.0)
        lstm_out = lstm_hidden * (2 if lstm_bidir else 1)  # 256 for Bi-LSTM(128)
        # Prof's head, scaled up to take 2*lstm_hidden input; output `fusion_ch`
    ...
    def forward(self, img, csv_window):
        ...
        h, _ = self.lstm(csv_window)                            # (B, SEQ_LEN, 2*lstm_hidden)
        # Bi-LSTM: use center segment to match v1's "center pool" convention
        center = csv_window.size(1) // 2
        csv_vec = self.csv_head(h[:, center, :])               # (B, fusion_ch)
...
model = FusionV3Model().to(device)
n_params = sum(p.numel() for p in model.parameters())
print(f"FusionV3Model parameters: {n_params:,}")
""")
...
add_row("fusion_v3 (prof data + v1 capacity + focal)", RUN_DIR / "test_metrics.json")
add_row("fusion_v2 (prof-style LSTM + focal)",
...
add_row("lstm_sweep winner (hidden_96, segments only)",
        SWEEP_DIR / "hidden_96" / "test_metrics.json")

cmp = pd.DataFrame(rows)
cmp.to_csv(RUN_DIR / "summary_vs_v1.csv", index=False)
