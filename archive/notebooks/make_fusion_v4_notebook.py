"""
Generate fusion_v4.ipynb -- deep fusion (U-Net + LSTM) where the CSV
branch is **hot-loaded from the sweep winner's trained weights**
(`runs/lstm_sweep/hidden_96/best.pt`) and **frozen**, so only the U-Net,
the new 16->fusion_ch mapping, the SE attention, and the fusion head
train.

Same architecture as fusion_v2:
    uni-LSTM(hidden=96), 1 layer, dropout=0.4
    Input: 5-segment window over 8 engineered features
But unlike v2, we don't train from scratch. The LSTM and the first two
Dense(16, elu) layers come pretrained from the LSTM-only sweep winner
(test mIoU 0.6978 on the segment task), and stay frozen during fusion
training.

    Loss: CategoricalFocalLoss(alpha=[0.05,0.45,0.60], gamma=2.0)
    LR:   1e-4 (matches v1 -- stable for ResNet-18 fine-tuning)

Outputs in runs/fusion_v4/:
  - best.pt, final.pt, metrics.csv
  - test_metrics.json
  - confmat.png (prof-style %)
  - loss_curve.png
  - summary_vs_all.csv -- v4 vs v3 vs v2 vs v1 vs LSTM-only winner
"""

from pathlib import Path
import nbformat as nbf

OUT = Path(__file__).parent / "fusion_v4.ipynb"
...
# =========================================================================
md(r"""# Deep Fusion v4 -- hot-load sweep-winner LSTM + freeze

The CSV branch architecture matches the LSTM-only sweep winner
(`lstm_sweep/hidden_96`, test mIoU 0.6978). v2 used the same
architecture but trained from random init. v4 instead **loads the
sweep winner's trained weights** into the LSTM and the first two
Dense(16, elu) layers, and **freezes them**. Only the U-Net, the
final 16->fusion_ch mapping, the SE attention, and the fusion head
train. This is the "two pretrained experts get glued together"
pattern -- like a CLIP-style composition.
...
| component | v3 (current best) | v4 (this notebook) |
|---|---|---|
| CSV branch arch | Bi-LSTM(128)x2 | **uni-LSTM(96)x1 (= sweep winner)** |
| CSV branch init | random | **loaded from lstm_sweep/hidden_96** |
| CSV branch trains | yes | **FROZEN** |
| CSV window | 5 segments | 5 segments |
| CSV features | 8 engineered | 8 engineered |
| Loss | Focal alpha=[.05,.45,.60] gamma=2.0 | same |
| Optimizer LR | 1e-4 | 1e-4 |
| Image branch (U-Net) | ImageNet pretrain, trainable | same |
...
BATCH_SIZE   = 32
EPOCHS       = 30
LR           = 1e-4                   # stable for ResNet-18 fine-tuning
WEIGHT_DECAY = 1e-4
PATIENCE     = 8
NUM_WORKERS  = 4
...
model = FusionV4Model().to(device)
n_params = sum(p.numel() for p in model.parameters())
print(f"FusionV4Model parameters: {n_params:,}")
""")

# -------- 6b. Hot-load sweep winner weights + freeze ------------------
md(r"""## 6b. Hot-load sweep-winner LSTM weights and freeze

We load `runs/lstm_sweep/hidden_96/best.pt` and copy its weights into:

| sweep state_dict key | fusion model dest | shape |
|---|---|---|
| `lstm.*` (4 tensors) | `lstm.*` | identical |
| `head.1.weight/bias` (Linear 96->16) | `csv_head.1.weight/bias` | identical |
| `head.4.weight/bias` (Linear 16->16) | `csv_head.4.weight/bias` | identical |
| `head.7.weight/bias` (Linear 16->3 logits) | *dropped* (we project 16->fusion_ch instead) |

Then we freeze every parameter we just loaded (LSTM + `csv_head.1.*` +
`csv_head.4.*`). The remaining trainable params are: the U-Net, the
final `csv_head.7.*` (16->fusion_ch=16), the SE attention, and the
fusion head.""")
...
# Freeze LSTM + first two Dense layers
FROZEN_PREFIXES = ("lstm.", "csv_head.1.", "csv_head.4.")
...
print(f"frozen fraction:   {n_frozen / (n_frozen + n_trainable):.2%}")
""")
...
print(f"optimizing {len(trainable_params)} parameter tensors")
...
add_row("fusion_v4 (frozen sweep-winner LSTM + U-Net)", RUN_DIR / "test_metrics.json")
